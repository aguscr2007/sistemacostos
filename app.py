...
# Localidades/barrios → partido GBA
LOCALIDAD_A_PARTIDO = {
    # San Martín
    "general san martin": "san martin", "general san martín": "san martin",
    "caseros": "tres de febrero", "villa adelina": "san isidro",
    "boulogne": "san isidro", "villa ballester": "san martin",
    "billinghurst": "san martin", "loma hermosa": "tres de febrero",
    ...
    "gregorio de laferrere": "la matanza", "gonzalez catan": "la matanza",
    "gonzález catán": "la matanza", "san justo": "la matanza",
    "villa luzuriaga": "la matanza",
    # Tigre
    "rincon de milberg": "tigre", "rincón de milberg": "tigre",
    "benavidez": "tigre", "benavídez": "tigre", "virreyes": "san fernando",
    # San Isidro
    "olivos": "vicente lopez", "florida": "vicente lopez",
    "florida oeste": "vicente lopez", "munro": "vicente lopez",
    "carapachay": "vicente lopez",
    # Berazategui
    "dock sud": "avellaneda", "sarandi": "avellaneda", "sarandí": "avellaneda",
    "san andres": "san isidro", "san andrés": "san isidro",
    "bella vista": "san miguel", "villa ballester": "san martin",
}
...
def procesar_reporte(df_ml, df_costos):
    ...
    for _, row in df_ml.iterrows():
        ...
        if costo_producto is None and not es_cancelada:
            sin_costo.append({"ID publicación": id_pub or "(sin ID)", "Título": titulo[:60]})
        ...
        # Para acuerdo con comprador: usar ingresos si total es 0
        ...
        if es_cancelada:
            devoluciones.append({
                ...
                "costo_envio_vuelta": 0.0, "perdida_total": anulacion,
...
with tab1:
    ...
    if archivo_ml and archivo_costos:
        ...
        if zonas_faltantes:
            st.warning(f"⚠️ Zonas Flex sin precio configurado: {', '.join(zonas_faltantes)}")
